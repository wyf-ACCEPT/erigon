...
func (s *RoSnapshots) ViewSingleFile(t snaptype.Type, blockNum uint64) (segment *VisibleSegment, ok bool, close func()) {
	...
	fmt.Printf("alex0: %d\n", len(segmentRotx.VisibleSegments))
	if len(segs.VisibleSegments) > 0 {
		fmt.Printf("alex1: last file %d-%d\n", segs.VisibleSegments[len(segs.VisibleSegments)-1].from, segs.VisibleSegments[len(segs.VisibleSegments)-1].to)
	}
