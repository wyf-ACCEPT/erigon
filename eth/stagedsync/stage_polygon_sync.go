...
func SpawnPolygonSyncStage(
	...
	if err := cfg.service.Run(ctx, txc, stageState, unwinder); err != nil {
		return err
	}
...
type polygonSyncStageTxAction struct {
	apply func(txc wrap.TxContainer) error
}
...
func (s *polygonSyncStageService) Run(ctx context.Context, txc wrap.TxContainer, stageState *StageState, unwinder Unwinder) error {
...
func (s polygonSyncStageCheckpointStore) GetLastEntityId(ctx context.Context) (uint64, bool, error) {
	...
	r, err := awaitTxAction(ctx, s.txActionStream, func(txc wrap.TxContainer, responseStream chan<- response) error {
		id, ok, err := s.checkpointReader.LastCheckpointId(ctx, txc.Tx)
...
func (s polygonSyncStageCheckpointStore) GetEntity(ctx context.Context, id uint64) (*heimdall.Checkpoint, bool, error) {
	...
	r, err := awaitTxAction(ctx, s.txActionStream, func(txc wrap.TxContainer, responseStream chan<- response) error {
		v, err := s.checkpointReader.Checkpoint(ctx, txc.Tx, id)
...
func (s polygonSyncStageCheckpointStore) PutEntity(ctx context.Context, id uint64, entity *heimdall.Checkpoint) error {
	...
	r, err := awaitTxAction(ctx, s.txActionStream, func(txc wrap.TxContainer, responseStream chan<- response) error {
		responseStream <- response{err: txc.Tx.Put(kv.BorCheckpoints, k[:], v)}
		return nil
	})
	if err != nil {
		return err
...
func (s polygonSyncStageCheckpointStore) RangeFromBlockNum(ctx context.Context, blockNum uint64) ([]*heimdall.Checkpoint, error) {
	...
	r, err := awaitTxAction(ctx, s.txActionStream, func(txc wrap.TxContainer, responseStream chan<- response) error {
		...
		r, err := blockRangeEntitiesFromBlockNum(txc.Tx, kv.BorCheckpoints, makeEntity, blockNum)
...
func (s polygonSyncStageMilestoneStore) GetLastEntityId(ctx context.Context) (uint64, bool, error) {
	...
	r, err := awaitTxAction(ctx, s.txActionStream, func(txc wrap.TxContainer, responseStream chan<- response) error {
		id, ok, err := s.milestoneReader.LastMilestoneId(ctx, txc.Tx)
		responseStream <- response{id: id, ok: ok, err: err}
		return nil
	})
	if err != nil {
...
func (s polygonSyncStageMilestoneStore) GetEntity(ctx context.Context, id uint64) (*heimdall.Milestone, bool, error) {
	...
	r, err := awaitTxAction(ctx, s.txActionStream, func(txc wrap.TxContainer, responseStream chan<- response) error {
		v, err := s.milestoneReader.Milestone(ctx, txc.Tx, id)
		responseStream <- response{v: v, err: err}
		return nil
	})
	if err != nil {
...
func (s polygonSyncStageMilestoneStore) PutEntity(ctx context.Context, id uint64, entity *heimdall.Milestone) error {
	...
	r, err := awaitTxAction(ctx, s.txActionStream, func(txc wrap.TxContainer, responseStream chan<- response) error {
		responseStream <- response{err: txc.Tx.Put(kv.BorMilestones, k[:], v)}
		return nil
	})
	if err != nil {
		return err
...
func (s polygonSyncStageMilestoneStore) RangeFromBlockNum(ctx context.Context, blockNum uint64) ([]*heimdall.Milestone, error) {
	...
	r, err := awaitTxAction(ctx, s.txActionStream, func(txc wrap.TxContainer, responseStream chan<- response) error {
		...
		r, err := blockRangeEntitiesFromBlockNum(txc.Tx, kv.BorMilestones, makeEntity, blockNum)
		responseStream <- response{result: r, err: err}
		return nil
	})
	if err != nil {
...
func (s polygonSyncStageSpanStore) GetLastEntityId(ctx context.Context) (id uint64, ok bool, err error) {
	...
	r, err := awaitTxAction(ctx, s.txActionStream, func(txc wrap.TxContainer, responseStream chan<- response) error {
		id, ok, err := s.spanReader.LastSpanId(ctx, txc.Tx)
		responseStream <- response{id: id, ok: ok, err: err}
		return nil
	})
	if err != nil {
...
func (s polygonSyncStageSpanStore) GetEntity(ctx context.Context, id uint64) (*heimdall.Span, bool, error) {
	...
	r, err := awaitTxAction(ctx, s.txActionStream, func(txc wrap.TxContainer, responseStream chan<- response) error {
		v, err := s.spanReader.Span(ctx, txc.Tx, id)
		responseStream <- response{v: v, err: err}
		return nil
	})
	if err != nil {
...
func (s polygonSyncStageSpanStore) PutEntity(ctx context.Context, id uint64, entity *heimdall.Span) error {
	...
	r, err := awaitTxAction(ctx, s.txActionStream, func(txc wrap.TxContainer, responseStream chan<- response) error {
		responseStream <- response{err: txc.Tx.Put(kv.BorSpans, k[:], v)}
		return nil
	})
	if err != nil {
		return err
...
func (s polygonSyncStageSpanStore) RangeFromBlockNum(ctx context.Context, blockNum uint64) ([]*heimdall.Span, error) {
	...
	r, err := awaitTxAction(ctx, s.txActionStream, func(txc wrap.TxContainer, responseStream chan<- response) error {
		...
		r, err := blockRangeEntitiesFromBlockNum(txc.Tx, kv.BorSpans, makeEntity, blockNum)
		responseStream <- response{result: r, err: err}
		return nil
	})
	if err != nil {
...
func (e *polygonSyncStageExecutionEngine) InsertBlocks(ctx context.Context, blocks []*types.Block) error {
	...
	r, err := awaitTxAction(ctx, e.txActionStream, func(txc wrap.TxContainer, responseStream chan<- response) error {
		responseStream <- response{err: e.insertBlocks(blocks, txc.Tx)}
		return nil
	})
	if err != nil {
		return err
...
func (e *polygonSyncStageExecutionEngine) UpdateForkChoice(ctx context.Context, tip *types.Header, _ *types.Header) error {
	...
	r, err := awaitTxAction(ctx, e.txActionStream, func(txc wrap.TxContainer, responseStream chan<- response) error {
		err := e.updateForkChoice(ctx, txc, tip)
...
func (e *polygonSyncStageExecutionEngine) updateForkChoice(ctx context.Context, txc wrap.TxContainer, tip *types.Header) error {
	tx := txc.Tx
	...
	if len(badNodes) > 0 {
		...
		return e.unwinder.UnwindTo(badNode.number, ForkReset(badNode.hash), txc.Doms)
	}
...
func (e *polygonSyncStageExecutionEngine) CurrentHeader(ctx context.Context) (*types.Header, error) {
	...
	r, err := awaitTxAction(ctx, e.txActionStream, func(txc wrap.TxContainer, responseStream chan<- response) error {
		r, err := e.currentHeader(ctx, txc.Tx)
		responseStream <- response{result: r, err: err}
		return nil
	})
	if err != nil {
...
func awaitTxAction[T any](
	ctx context.Context,
	txActionStream chan<- polygonSyncStageTxAction,
	cb func(txc wrap.TxContainer, responseStream chan<- T) error,
) (T, error) {
	responseStream := make(chan T)
	txAction := polygonSyncStageTxAction{
		apply: func(txc wrap.TxContainer) error {
			return cb(txc, responseStream)
		},
	}
