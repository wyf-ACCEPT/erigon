...
func (opts MdbxOpts) Open(ctx context.Context) (kv.RwDB, error) {
	...
	if opts.label == kv.ChainDB {
		opts.log.Info("[db] open", "label", opts.label, "sizeLimit", opts.mapSize, "pageSize", opts.pageSize, "inMem", opts.inMem)
	} else {
		opts.log.Debug("[db] open", "label", opts.label, "sizeLimit", opts.mapSize, "pageSize", opts.pageSize, "inMem", opts.inMem)
	}
