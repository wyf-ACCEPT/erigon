...
func OpenIndex(indexFilePath string) (id *Index, err error) {
	...
	offset++
	fmt.Println("Processing", idx.fileName, "idx.enums", idx.enums, "idx.keyCount", idx.keyCount)
	if idx.enums && idx.keyCount > 0 {
		var size int
		idx.offsetEf, size = eliasfano32.ReadEliasFano(idx.data[offset:])
		offset += size
		fmt.Printf("%s: offsetEf size: %d, total size: %d, ratio: %.2f \n",
			idx.fileName, size, len(idx.data), float64(size)/float64(len(idx.data)))
